...
## 설계 철학 — 알고리즘은 모니터링, 결정은 인간

이 도구는 **매수/매도를 추천하지 않는다.** 코드는 "관찰 사실"만 말한다
(예: *"RSI 베어리시 다이버전스 발생", "200일선 +60% 이격 = 98백분위수"*).
무엇을 할지는 사람이 정한다.

이렇게 선을 그은 이유: 완전 자동매매 시스템은 보통 승률 50–60%에 그치고
buy-and-hold를 이기지 못하는 경우가 많다. 더 위험한 건, 사이클 판단이나
밸류에이션 해석을 자동화하면 **틀린 숫자에 확신을 갖게** 된다는 점이다.
그래서 숫자(백분위수)는 코드가, 판단(문장)은 사람이 채우는 **하이브리드** 구조다.

---

## 지금 수준에서 실제로 어디까지 되나 (정직한 표)
...
경제지표(FRED)와 펀더멘털 판단만 이후 단계 / 수동으로 남긴다.

### 매크로는 다 API로 끌어와야 하나?
아니다. 의사결정에 중요한 **금리·VIX·달러·유가는 yfinance로 키 없이** 끌어온다(80% 커버).
진짜 경제지표(CPI·GDP)만 FRED 무료 키가 필요하며, 이건 선택 사항이다.
